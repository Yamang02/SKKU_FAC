...
import BaseAdminController from '#src/domain/admin/controller/BaseAdminController.js';

export default class UserAdminController extends BaseAdminController {
    // 의존성 주입을 위한 static dependencies 정의
    static dependencies = ['UserAdminService'];

    constructor(userAdminService = null) {
        super('UserAdminController');
        ...
        if (userAdminService) {
            ...
            // 기존 방식 호환성 유지 (임시)
            throw new Error('UserAdminService가 주입되지 않았습니다.');
        }
    }
    ...
    async getManagementUserList(req, res) {
        return this.safeExecuteSSR(
            async () => {
                ...
                const result = await this.userAdminService.getUserList({
                    page: parseInt(page),
    ...
    async getManagementUserDetail(req, res) {
        return this.safeExecuteSSR(
            async () => {
                ...
                const user = await this.userAdminService.getUserDetail(userId);
    ...
    async updateManagementUser(req, res) {
        return this.safeExecuteSSR(
            async () => {
                ...
                await this.userAdminService.updateUser(userId, userData);
    ...
    async deleteManagementUser(req, res) {
        return this.safeExecuteSSR(
            async () => {
                const userId = req.params.id;
                await this.userAdminService.deleteUser(userId);
    ...
    async resetManagementUserPassword(req, res) {
        return this.safeExecuteSSR(
            async () => {
                const userId = req.params.id;
                const result = await this.userAdminService.resetUserPassword(userId);
